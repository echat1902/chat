...
def test_message(message):
    ...
    send_user_id = int(message["send_uesr_id"])
    lid = int(message["lid"])
    obj = get_obj_by_lid(lid)
    user_nick_name = get_pick_name(obj, send_user_id)  # 根据lid查询对象返回个人昵称或群昵称
    ...
    if msg_type == 3:  # 文件
        ...
        msg_dict = {"user_nick_name": user_nick_name, "pic_name": pic_name, "msg_type": msg_type,
                    "file_name": message["file_name"], "file_ext": ext, "filesize": message["file_size"]}
    elif msg_type == 2:  # 图片
        chatlist_content = "[图片]"
        content = message["send_msg"]
        msg_dict = {"user_nick_name": user_nick_name, "pic_name": pic_name, "send_msg": message["send_msg"],
                    "msg_type": msg_type}
    elif msg_type == 1:  # 文本
    ...
    socketio.emit('server_response', json.dumps(msg_dict), room=message['lid'], namespace='/flasksocketio')
